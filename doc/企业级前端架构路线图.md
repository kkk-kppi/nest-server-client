...
- [ ] 建立多环境 CI/CD（dev/test/stage/prod）
  ...
  - **当前进展**：已新增 `.github/workflows/multi-env-ci-cd.yml`，覆盖 `develop/test/stage/main` 与 `v*` tag 触发；落地 quality、build_artifact、deploy_dev/test/stage/prod 四环境作业，并支持 `workflow_dispatch` 手动触发与目标环境选择。
  - **优先级**：P0
