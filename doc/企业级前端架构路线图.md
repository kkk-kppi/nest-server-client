...
| Phase 2 / 2.2 | 建立 composables 约定与副作用治理                                       | 未开始 | 尚未沉淀通用 composables 模板与复用基线                    |
| Phase 2 / 2.3 | 搭建 axios 实例、拦截器与错误码处理中台                                 | 已完成 | 统一 HTTP 客户端、鉴权与错误处理已上线                     |
| Phase 2 / 2.3 | 建立 API 类型自动推导与 Mock 方案                                       | 未开始 | 当前以手写类型为主，自动生成与 MSW 未接入                  |
...
| Phase 3 / 3.1 | 建立 Cypress E2E，覆盖核心流程 10 条用例                                | 未开始 | E2E 框架与用例未落地                                       |
| Phase 3 / 3.1 | 建立 MR 模板与 Code Review 清单                                         | 未开始 | 模板与评审清单未落库                                       |
...
- [ ] 建立 composables 约定与副作用治理
  - **实施内容**：在 `src/shared/composables` 建立命名规则（`useXxx`）、参数与返回值类型模板、请求生命周期封装（loading/error/data）；示例位置：新增 `useAsyncState.ts`、`usePagination.ts`；验收标准：跨页面复用逻辑至少沉淀 3 个 composables，单测通过。
  - **优先级**：P1
  - **责任方**：前端（FE）
  - **预估工时**：2.0 人日
...
- [ ] 建立 API 类型自动推导与 Mock 方案
  - **实施内容**：优先方案 `openapi-typescript` + `orval`（有 OpenAPI 时）；无 OpenAPI 时使用手写 DTO + `zod` 校验；Mock 选型 `msw`（开发）+ `cypress fixtures`（E2E）；示例位置：`src/core/http/types.ts`、`src/mocks/handlers.ts`；验收标准：关键接口有类型约束与 mock 数据，前后端可并行开发。
  - **优先级**：P1
  - **责任方**：前端（FE）+ 架构（ARCH）
  - **预估工时**：3.0 人日
...
- [ ] 建立 MR 模板与 Code Review 清单（性能/安全/可访问性 20 条）
