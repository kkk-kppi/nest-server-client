...
- [ ] 建立多环境 CI/CD（dev/test/stage/prod）
  ...
  - **当前进展**：已新增 `.github/workflows/multi-env-ci-cd.yml`，覆盖 `develop/test/stage/main` 与 `v*` tag 触发；落地 quality、build_artifact、deploy_dev/test/stage/prod 四环境作业；`workflow_dispatch` 已支持 `deploy/rollback`、target 环境与 `rollback_version`；部署命令未配置时由“跳过”改为“失败阻断”，以保证发布可控与可追踪。
  - **待办（暂缓）**：配置 8 个环境命令 Secrets（`DEV/TEST/STAGE/PROD_DEPLOY_COMMAND` + `DEV/TEST/STAGE/PROD_ROLLBACK_COMMAND`），当前按你的要求先跳过，后续在 3.2.1 验收前补齐。
  - **优先级**：P0
