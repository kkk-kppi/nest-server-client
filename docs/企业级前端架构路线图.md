...
- [Docker 部署与运行指南](./Docker部署与运行指南.md)
- [Vite 分包策略与性能预算治理](./Vite分包策略与性能预算治理.md)
- [API MOCK 指南和接入说明](./API%20MOCK指南和接入说明.md)
- [E2E 测试与接入指南](./E2E测试与接入指南.md)
- [环境变量提交规范](./环境变量提交规范.md)
- [分支规范文档](./分支规范文档.md)
...
| Phase 3 / 3.2 | 建立多环境 CI/CD（dev/test/stage/prod）                                 | 进行中 | GitHub Actions 初版流水线已落地，待接入真实部署命令        |
| Phase 3 / 3.2 | 落地 Dockerfile 多阶段构建与安全基线                                    | 进行中 | 多阶段 Dockerfile 与 nginx 安全基线已落地，待接入镜像扫描  |
| Phase 3 / 3.2 | 执行 Vite 分包策略与性能预算治理                                        | 进行中 | manualChunks 与预算门禁已落地，待 stage 环境实测对齐目标   |
...
- [ ] 建立多环境 CI/CD（dev/test/stage/prod）
  ...
  - **当前进展**：已新增 `.github/workflows/multi-env-ci-cd.yml`，覆盖 `develop/test/stage/main` 与 `v*` tag 触发；落地 quality、build_artifact、deploy_dev/test/stage/prod 四环境作业；`workflow_dispatch` 已支持 `deploy/rollback`、target 环境、`rollback_version` 与 `dry_run`；可通过 `action=deploy + target=all + dry_run=true` 完成端到端链路验收闭环，同时保留真实发布场景的 Secrets 缺失失败阻断策略，确保上线可控。
  - **待办（生产发布前）**：配置 8 个环境命令 Secrets（`DEV/TEST/STAGE/PROD_DEPLOY_COMMAND` + `DEV/TEST/STAGE/PROD_ROLLBACK_COMMAND`），用于非 `dry_run` 的真实发布与回滚。
  - **优先级**：P0
...
- [ ] 落地 Dockerfile 多阶段构建与安全基线
  ...
  - **当前进展**：已新增根目录 `Dockerfile` 与 `nginx.conf`；构建阶段使用 `node:22-alpine` + `pnpm --frozen-lockfile`，运行阶段使用 `nginx:1.28-alpine`，默认监听 `8080`，以 `nginx` 非 root 用户运行，并添加常用安全响应头与 SPA 回退规则；已新增 `.dockerignore` 控制构建上下文；本地容器访问 `/` 与 `/healthz` 验证通过；配套文档见 [Docker 部署与运行指南](./Docker部署与运行指南.md)。
  - **优先级**：P0
  - **责任方**：运维（OPS）+ 前端（FE）
  - **预估工时**：2.0 人日
...
- [ ] 执行 Vite 分包策略与性能预算治理
  ...
  - **当前进展**：已在 `vite.config.ts` 落地 `manualChunks`（framework-vue/framework-routing-state/framework-http/vendor/mock-vendor）；`src/core/router/routes.ts` 仅保留基础路由，`Admin/Workspace` 改为 `dynamic-routes.ts` 登录后按角色注入并懒加载；构建期预算门禁已恢复默认阈值 `300/300 KiB`；通过将 `msw/@mswjs` 从首屏 `vendor` 中拆分到 `mock-vendor`，入口静态依赖收敛至约 `138.64 KiB` 并通过预算校验；已完成 Vite 插件聚合导出与分环境构建预设（`VITE_BUILD_PRESET`）；`multi-env-ci-cd` 已固化预算变量、在构建前解析注入 preset 并复用 preset 解析步骤，同时在 `quality` 阶段上传 `bundle-report` 产物，已补充 stage 验收操作清单、preset 解析优先级、环境变量覆盖顺序与排障指引；配套文档见 [Vite 分包策略与性能预算治理](./Vite分包策略与性能预算治理.md) 与 [多环境CI-CD接入说明](./多环境CI-CD接入说明.md)。
  - **优先级**：P1
  - **责任方**：前端（FE-Owner）+ 架构（ARCH）
  - **预估工时**：2.5 人日
